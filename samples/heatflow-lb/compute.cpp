...
void compute_new_values(DoubleMatrix &matrix, double *top_row, double *bottom_row)
{
	...
	// compute corners
	matrix.set2(0, 0, (top_row[0] + matrix.get(1, 0) + matrix.get(0, 1)) / 4.0);
	matrix.set2(x - 1, 0, (top_row[x-1] + matrix.get(x - 2, 0)
		+ matrix.get(x - 1, 1)) / 4.0);
	matrix.set2(0, y - 1, (bottom_row[0] + matrix.get(1, y - 1)
		+ matrix.get(0, y - 2)) / 4.0);
	matrix.set2(x - 1, y - 1, (bottom_row[x-1] + matrix.get(x - 2, y - 1)
...
void set_fixed_temp(DoubleMatrix &matrix, int full_size_y, int position, double temp)
{
	...
	if(source_y >= position && source_y < position + matrix.get_size_y()) {
		...
		matrix.set1(source_x, source_y, temp);
	}
}
