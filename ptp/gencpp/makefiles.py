...
def lib_path(path):
    return os.path.join("${KAIRA_LIBS}", path)
...
def prepare_makefile(project, config, directory):
    ...
    makefile.set("MPILIBDIR",
                 "-L{0} {1}".format(lib_path(paths.CAILIE_MPI_LIB_DIR),
                                    " ".join("-L" + s for s in config["libdir"])))
...
def write_statespace_makefile(project, directory):
    config = create_makefile_config()
    config["include"].append(lib_path(paths.CAVERIF_DIR))
    config["libdir"].append(lib_path(paths.CAVERIF_DIR))
...
def write_server_makefile(project, directory):
    ...
    config = create_makefile_config()
    config["include"].append(lib_path(paths.CASERVER_DIR))
    config["libdir"].append(lib_path(paths.CASERVER_DIR))
...
def write_library_makefile(project, directory, rpc=False, octave=False):
    ...
    if rpc:
        config["include"].append(lib_path(paths.CACLIENT_DIR))
        config["libdir"].append(lib_path(paths.CACLIENT_DIR))
